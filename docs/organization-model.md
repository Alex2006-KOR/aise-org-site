...
::: tip 읽는 자리 — 동작 원리 (1/5)
[동작 원리](/how-it-works) 대분류의 첫 글입니다. 앞 대분류가 "왜"를 끝냈으니, 여기서부터는
**"어떻게"** 입니다. 이 대분류는 정적인 구조에서 시작해 점점 움직이는 쪽으로 가는데, 이
페이지가 그 출발점 — **무엇이 있는지**(참모·부서·역할의 배치) 를 먼저 그립니다. 이름들이
여기서 다 나오므로 이 페이지가 먼저여야 합니다. 다음 [Staff & Governance](/staff-governance)는
그 배치도에서 가장 설명이 필요한 자리인 참모를 확대합니다.
:::
