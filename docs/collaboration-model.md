...
::: tip 읽는 자리 — 동작 원리 (4/5)
[동작 원리](/how-it-works) 대분류의 네 번째 글입니다. 앞 [Lifecycle](/lifecycle)이 부서 **하나**의
시간축을 따라갔다면, 이 페이지는 **여러 개가 같은 순간에 굴러갈 때** 무슨 일이 생기는지를
다룹니다 — 누가 실행 그래프를 조립하는가, 서로를 기다리는 두 역할을 어떻게 풀어주는가. 다음
[Operator vs Meta Mode](/operator-vs-meta-mode)가 이 대분류의 마지막 안전장치입니다.
:::
