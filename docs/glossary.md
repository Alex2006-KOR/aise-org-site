...
- **Project Record** — PM의 상세 연속성 기억(`project-record.md`), 업무참모가 남기는 미흡수
  지시함(`directive.md`), 업무참모에게 보내는 경량 보고(`execution.md`)로 구성됩니다. 파일마다
  쓰는 주체가 정확히 한 명이고, 그렇게 갈라진 사연은
  [세션을 넘어 이어가기](/handoff) 참고. (이 세 번째 파일은 2026-08-26에 `report.md`로 처음
  만들어졌다가 이틀 뒤 실행 도구의 파일명 가드와 충돌해 `execution.md`로 개칭됐습니다.)
- **run** — 한 번의 실행. 부서는 여러 번의 독립적인 run에 걸쳐 존재하고, run들은 서로 기억을
  공유하지 않습니다 — 매번 위 세 파일에서 다시 부트스트랩합니다.
  [세션을 넘어 이어가기](/handoff) 참고.
- **`/aise:op` · `/aise:meta` · `/aise:handoff`** — 각각 Operator 모드 선언, Meta 모드 선언,
  그리고 지금 시점의 연속성 기록 쓰기. [일을 맡기는 법](/usage) 참고.
- **Ledger** — `project-record.md` 안의 추가 전용(append-only) 이력. 흡수한 지시의 원문 인용과
  각 run의 산출물이 여기 남습니다 — 지시함을 작게 유지할 수 있는 이유입니다.
