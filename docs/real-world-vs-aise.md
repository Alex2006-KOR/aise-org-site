...
**자기 결정 기록에 직접 적어둔 비교 지점**입니다. 각 항목 끝에 원본 파일 경로가 붙어 있으니
직접 대조해 보셔도 됩니다.

## 한눈에 보는 일곱 축

| 축 | 흔히 하는 방식 | AISE | 이 대조가 적힌 곳 |
|---|---|---|---|
| **1. 조직의 기억** | 사람의 머리, 흩어진 채팅 로그, 도구가 주는 개인 메모리 | 저장소에 커밋된 파일. `git clone` 하나로 재구성되지 않으면 잘못된 자리 | `2026-07-07-org-memory-must-be-project-local.md` |
| **2. 조직도를 언제 그리나** | 도메인을 정하고 역할을 미리 설계해 자리를 채운다 | 비어 있는 채로 시작. 실제 업무가 빈자리를 드러낼 때만 채용 | `2026-07-07-recruit-on-demand-phase-1.md` |
| **3. 구성원의 정체성** | 사람 한 명이 한 자리를 차지한다(인재풀) | 역할은 **클래스**. 부서에 묶이지 않고, 동시에 몇 군데서 쓰여도 경합이 없다 | `2026-07-09-role-is-a-class-not-an-instance.md` |
| **4. 경험이 쌓이는 자리** | 개인이 숙련된다. 인사고과가 그걸 기록한다 | 개인이 아니라 **조직**에 쌓는다. 개인 숙련도 메커니즘은 두 번 연속 "아직 만들지 않음" | `2026-07-07-role-proficiency-idea-deferred.md`, `2026-09-02-role-proficiency-idea-still-deferred.md` |
| **5. 보고선** | 급한 건 담당자에게 직접 말한다 | 사소한 일이라도 예외 없이 참모를 거친다(no skip-level) | `2026-07-07-line-staff-org-model.md` |
| **6. 권한** | 세분화된 접근제어 매트릭스를 짠다 | 거친 단위의 승인 + 판단. 진짜 경계는 애플리케이션이 아니라 OS/인프라 층 | `2026-07-09-provisioning-scope-is-a-judgment-not-a-mechanism.md`, `2026-07-14-folder-scoped-access-control-deferred.md` |
| **7. 세션을 이어받는 법** | 진행 중인 컨텍스트를 다음 실행에 그대로 물려준다 | 물려주지 않는다. 매번 **기록에서 다시 도출**한다 | `2026-09-08-stateless-pm-sidesteps-handoff-tax.md` |
...
flowchart TB
  ...
  C -->|"있다"| B["만든다"]
  C -->|"아직 없다"| D["<b>안 만든다</b><br/>— 대신 '왜 안 만들었는지'와<br/>'언제 다시 볼지'를 기록으로 남긴다"]
...
**이 페이지를 직접 확인해 보려면.** 위 표의 마지막 열이 전부 실제 파일 경로입니다.
